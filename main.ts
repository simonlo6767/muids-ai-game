...
function showResult(text: string) {
    let timePlayed = (game.runtime() - startTime) / 1000

    game.showLongText(
        text + "\n\n" +
        "\nTime: " + Math.round(timePlayed) + "s" +
        "\nGhosts Used: " + ghostUsed +
        "\nCoins: " + coins,
        DialogLayout.Center
    )
...
function playBGM() {
    ...
    control.runInParallel(function () {
        while (true) {
            music.playMelody("C5 A G A C5 A G A", 120)
